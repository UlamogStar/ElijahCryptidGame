...
public class HandAnim : MonoBehaviour
{
    ...
    private const string ANIM_LAYER_NAME_POINT = "Point Layer",
        ANIM_LAYER_NAME_THUMB = "Thumb Layer",
        ANIM_PARAM_NAME_FLEX = "Flex",
        ANIM_PARAM_NAME_POSE = "Pose",
        ANIM_PARAM_NAME_PINCH = "Pinch";

    private const float INPUT_RATE_CHANGE = 20.0f;

    private int m_animLayerIndexThumb = -1,
        m_animLayerIndexPoint = -1,
        m_animParamIndexFlex = -1,
        m_animParamIndexPose = -1,
        m_animParamIndexPinch = -1;
    ...
    private void Update()
    {
        ...
        m_pointBlend =
            Mathf.Clamp01(m_pointBlend + (Time.deltaTime * INPUT_RATE_CHANGE) * (m_isPointing ? 1.0f : -1.0f));
        m_thumbsUpBlend =
            Mathf.Clamp01(m_thumbsUpBlend + (Time.deltaTime * INPUT_RATE_CHANGE) * (m_isGivingThumbsUp ? 1.0f : -1.0f));
